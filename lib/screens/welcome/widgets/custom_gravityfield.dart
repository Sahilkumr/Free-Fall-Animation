...
class CustomGravityField extends StatefulWidget {
  ...
  @override
  _CustomGravityFieldState createState() => _CustomGravityFieldState();
}

class _CustomGravityFieldState extends State<CustomGravityField>
  ...
  List<double> xConPos = [30, 100, 200, 150];
            ...
            child: Stack(
              children: [
                CustomPaint(
                  ...
                  // This container is necessary for size constraints
                ),
              ],
            ),
          );
    ...
    for (int i = 0; i < objList.length; i++) {
      for (int j = i + 1; j < objList.length; j++) {
        if (getDistance(objList[i], objList[j]) <
          ...
          objList[i].subXpos(
              (viScala / (viScala + vjScala)) * correctDistance * ripNorVec[0]);
          objList[i].subYpos(
              (viScala / (viScala + vjScala)) * correctDistance * ripNorVec[1]);

          objList[j].subXpos(
              (vjScala / (viScala + vjScala)) * correctDistance * rjpNorVec[0]);
          objList[j].subYpos(
              (vjScala / (viScala + vjScala)) * correctDistance * rjpNorVec[1]);
...
class _paint extends CustomPainter {
  ...
  final List conTitles;

  _paint({
    required this.pathList,
    required this.paintList,
    required this.conTitles,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < pathList.length; i++) {
      ...
      canvas.drawPath(pathList[i][0], paintList[i][0]..color = colors[i]);
      canvas.drawPath(pathList[i][1], paintList[i][1]);
      // Calculate the bounding box of the path
      Rect pathBounds = pathList[i][0].getBounds();

      // Get the center of the bounding box
      Offset center = pathBounds.center;

      // Adjust the position of the text relative to the center of the bounding box
      ...
      textPainter.paint(
        canvas,
        Offset(
          center.dx - textPainter.width / 2, // Adjust X position
          center.dy - textPainter.height / 2, // Adjust Y position
        ),
      );
    }
